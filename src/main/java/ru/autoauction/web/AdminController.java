...
public class AdminController {
  ...
  public Map<String,Long> create(@RequestParam String title,@RequestParam String description,@RequestParam(required=false) String vin,@RequestParam(required=false) String mileage,@RequestParam(required=false) Integer ownersCount,@RequestParam(required=false) String engineNumber,@RequestParam(required=false) String fuelType,@RequestParam(required=false) String engineVolume,@RequestParam(required=false) String horsepower,@RequestParam(required=false) String transmission,@RequestParam(required=false) String driveType,@RequestParam(required=false) String autotecaUrl,@RequestParam long startingPrice,@RequestParam long bidStep,@RequestParam long durationMinutes,@RequestParam(defaultValue="true") boolean publish,@RequestPart(required=false) List<MultipartFile> media,HttpSession s) throws IOException {
    current.requireSuperAdmin(s);if(startingPrice<0||bidStep<=0||durationMinutes<1||durationMinutes>525_600||ownersCount!=null&&ownersCount<0)throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Проверьте цены, владельцев и длительность");Lot l=new Lot();l.title=title;l.description=description;l.vin=blankToNull(vin);l.mileage=blankToNull(mileage);l.ownersCount=ownersCount;l.engineNumber=blankToNull(engineNumber);l.fuelType=blankToNull(fuelType);l.engineVolume=blankToNull(engineVolume);l.horsepower=blankToNull(horsepower);l.transmission=blankToNull(transmission);l.driveType=blankToNull(driveType);l.autotecaUrl=blankToNull(autotecaUrl);l.startingPrice=startingPrice;l.currentPrice=startingPrice;l.bidStep=bidStep;l.startsAt=Instant.now();l.endsAt=l.startsAt.plus(Duration.ofMinutes(durationMinutes));l.status=publish?LotStatus.LIVE:LotStatus.DRAFT;lots.save(l);
