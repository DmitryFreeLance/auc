...
public class AuctionController {
  ...
  @PostMapping("/lots/{id}/bids") public BidDto bid(@PathVariable long id,HttpSession s){AppUser bidder=current.requireRegistered(s);AuctionService.BidResult result=auction.placeBid(id,bidder);events.bidUpdated(id,result.bid().amount,bidder.id);if(result.previousLeader()!=null&&!result.previousLeader().id.equals(bidder.id))bot.sendOutbid(result.previousLeader().maxUserId,result.lot().title,result.bid().amount);return BidDto.of(result.bid());}
  @GetMapping(value="/lots/{id}/events",produces=MediaType.TEXT_EVENT_STREAM_VALUE) public ResponseEntity<SseEmitter> events(@PathVariable long id,HttpSession s){current.requireRegistered(s);if(!lots.existsById(id))throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Лот не найден");return ResponseEntity.ok().cacheControl(CacheControl.noCache()).header("X-Accel-Buffering","no").body(events.subscribe(id));}
  @GetMapping(value="/events",produces=MediaType.TEXT_EVENT_STREAM_VALUE) public ResponseEntity<SseEmitter> globalEvents(HttpSession s){current.requireRegistered(s);return ResponseEntity.ok().cacheControl(CacheControl.noCache()).header("X-Accel-Buffering","no").body(events.subscribeGlobal());}
  ...
  private LotDto dto(Lot l,AppUser viewer){List<Bid> all=bids.findByLotIdOrderByCreatedAtDesc(l.id);Bid top=all.stream().max(Comparator.comparingLong(b->b.amount)).orElse(null);Long my=all.stream().filter(b->b.user.id.equals(viewer.id)).map(b->b.amount).max(Long::compare).orElse(null);LotStatus status=l.status;if(status==LotStatus.LIVE&&!Instant.now().isBefore(l.endsAt))status=LotStatus.FINISHED;return new LotDto(l.id,l.title,l.description,l.vin,l.mileage,l.autotecaUrl,l.startingPrice,l.currentPrice,l.bidStep,l.startsAt,l.endsAt,status,l.media.stream().map(m->new MediaDto(m.id,m.url,m.type)).toList(),all.size(),top==null?null:top.user.id,top==null?null:top.user.name,my);}
